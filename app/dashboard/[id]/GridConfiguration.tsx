...
export default function GridConfiguration({ capacity }: OptimizationResult) {
  const { PV, battery, diesel } = capacity
  const microGridConfig = {
    data: [
      {
        type: 'pie',
        values: [PV, battery, diesel].map(x => parseFloat(x.toFixed(2))),
  ...
  return (
    <div className='h-96 bg-white shadow-lg rounded-lg p-4 pb-12 border border-gray-200'>
